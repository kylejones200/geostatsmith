...
class MaternModel(VariogramModelBase):
    ...
    A flexible model controlled by smoothness parameter nu.
    Special cases:
    - nu -> inf: Gaussian model
    - nu = 0.5: Exponential model

    Formula:
    gamma(h) = nugget + (sill - nugget) * [1 - (2^(1-nu)/Gamma(nu)) * (h/a)^nu * K_nu(h/a)]
...
class HoleEffectModel(VariogramModelBase):
    ...
    Formula:
    gamma(h) = nugget + (sill - nugget) * [1 - sin(h/a)/(h/a)] for h > 0
    gamma(0) = nugget
...
class CubicModel(VariogramModelBase):
    ...
    Formula:
    gamma(h) = nugget + (sill - nugget) * [7*(h/a)^2 - 8.75*(h/a)^3 + 3.5*(h/a)^5 - 0.75*(h/a)^7] for 0 < h <= a
    gamma(h) = sill for h > a
    """
...
class StableModel(VariogramModelBase):
    ...
    Formula:
    gamma(h) = nugget + (sill - nugget) * [1 - exp(-(h/a)^s)]
    ...
    def __init__(
        ...
        super().__init__(nugget=nugget, sill=sill, range_param=range_param)
        if not (0 < shape <= 2):
            raise ValueError("shape must be in (0, 2]")
