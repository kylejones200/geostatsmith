...
the space-time variogram is:
 gamma(h, u) = 0.5 * E[(Z(s, t) - Z(s+h, t+u))^2]
...
Key concepts:
1. Separability: gamma(h, u) = gamma_s(h) * gamma_t(u)
   - Simplest case: spatial and temporal structures independent

2. Non-separability: More complex interaction
   - Product-sum: gamma(h,u) = (C_s*gamma_t(u) + C_t*gamma_s(h) + gamma_s(h)*gamma_t(u))
   - Cressie-Huang: More general non-separable models

3. Full symmetry: gamma(h, u) = gamma(h, -u) (time-reversible)
...
class SpaceTimeVariogramModel(ABC):
    ...
    def is_separable(self) -> bool:
        """Whether the model is separable"""
...
class SeparableModel(SpaceTimeVariogramModel):
    ...
    def __init__(self, spatial_model: Callable, temporal_model: Callable):
        ...
        spatial_model : callable
            Spatial variogram model gamma_s(h)
        temporal_model : callable
            Temporal variogram model gamma_t(u)
        """
    ...
    def __call__(
        ...
        # Separable model: gamma(h,u) = gamma_s(h) + gamma_t(u) - gamma_s(h)*gamma_t(u)
        # This ensures gamma(0,0) = 0
        gamma = gamma_s + gamma_t - gamma_s * gamma_t

        return gamma

    def is_separable(self) -> bool:
        """Separable model is always separable"""
        return True
...
class ProductSumModel(SpaceTimeVariogramModel):
    ...
    def __call__(
        ...
        # Product-sum: gamma(h,u) = C_s*gamma_t + C_t*gamma_s + k*gamma_s*gamma_t
        gamma = self.C_s * gamma_t + self.C_t * gamma_s + self.k * gamma_s * gamma_t

        return gamma

    def is_separable(self) -> bool:
        """Product-sum model is separable only if k=0"""
        return self.k == 0.0
...
class GneitingModel(SpaceTimeVariogramModel):
    ...
    def __init__(
        ...
        spatial_smoothness : float
            Spatial smoothness parameter (kappa in Matern)
        """
        self.sigma2 = sigma2
        self.a0 = a0
        self.alpha = alpha
    ...
    def _spatial_correlation(
        ...
        # Can be extended to other forms (Gaussian, Matern, etc.)
    ...
    def __call__(
        ...
        # Variogram: gamma = sigma^2 - C
        gamma = self.sigma2 - cov

        return gamma

    def is_separable(self) -> bool:
        """Gneiting model is non-separable"""
        return False
...
def create_spacetime_model(
    ...
    **kwargs
    ...
    if model_type in ["separable", "product_sum"]:
        ...
        return models[model_type](spatial_model, **kwargs)
