 ...
 ... )
    ...
    with nc.Dataset(filename, 'r') as dataset:
        ...
        if y_var not in dataset.variables:
            raise KeyError(f"Variable '{y_var}' not found in dataset")
        if z_var not in dataset.variables:
            raise KeyError(f"Variable '{z_var}' not found in dataset")
...
def read_geojson(
    ...
    if z_property not in gdf.columns:
        raise KeyError(f"Property '{z_property}' not found in GeoJSON file")
