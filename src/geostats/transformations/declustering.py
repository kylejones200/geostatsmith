...
def cell_declustering(
    ...
    cell_sizes : np.ndarray, optional
        Cell sizes to test
    n_sizes : int
        Number of cell sizes to test if cell_sizes not provided

    Returns
    -------
    weights : np.ndarray
        Declustering weights for each sample
    results : dict
...
def detect_clustering(
    ...
    Uses cumulative distribution of distance to nearest neighbor.
