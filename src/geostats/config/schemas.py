...
class ProjectConfig(BaseModel):
    ...
    author: str | None = Field(None, description="Analysis author")
...
class DataConfig(BaseModel):
    ...
    z_secondary: str | None = Field(
        None, description="Secondary variable for cokriging"
    )
    filter_column: str | None = Field(None, description="Column to filter by")
    filter_value: Any | None = Field(None, description="Value to filter for")
...
class PreprocessingConfig(BaseModel):
    ...
    outlier_method: Literal["iqr", "zscore", "isolation_forest"] = Field(
        "iqr", description="Outlier detection method"
    )
    outlier_threshold: float = Field(3.0, description="Threshold for outlier detection")

    transform: Literal["log", "boxcox", "normal_score", "sqrt"] | None = Field(
        None, description="Data transformation"
    )
    transform_params: dict[str, Any] | None = Field(
        None, description="Transform parameters"
    )

    declustering: bool = Field(False, description="Apply declustering weights")
    declustering_method: Literal["cell", "polygonal"] = Field(
        "cell", description="Declustering method"
    )

    handle_negatives: Literal["shift", "remove", "absolute"] = Field(
        "shift", description="How to handle negative values for log/boxcox"
    )


class VariogramConfig(BaseModel):
    n_lags: int = Field(15, ge=5, le=50, description="Number of lag bins")
    max_lag: float | None = Field(
        None, description="Maximum lag distance (auto if None)"
    )
    lag_tolerance: float | None = Field(None, description="Lag tolerance")

    estimator: Literal["matheron", "cressie", "dowd", "madogram"] = Field(
        "matheron", description="Variogram estimator"
    )
    ...
    ] = Field(["spherical", "exponential", "gaussian"], description="Models to try")
    ...
    fit_criterion: Literal["rmse", "mae", "r2", "aic"] = Field(
        "rmse", description="Model selection criterion"
    )

    # Manual parameters (used if auto_fit=False)
    manual_model: str | None = Field(None, description="Manual model selection")
    manual_nugget: float | None = Field(None, description="Manual nugget")
    manual_sill: float | None = Field(None, description="Manual sill")
    manual_range: float | None = Field(None, description="Manual range")

    # Directional variography
    check_anisotropy: bool = Field(False, description="Check for anisotropy")
    anisotropy_angles: list[float] = Field(
        [0, 45, 90, 135], description="Angles for directional variograms"
    )
...
class NeighborhoodConfig(BaseModel):
    ...
    search_radius: float | None = Field(
        None, description="Search radius (auto if None)"
    )
...
class GridConfig(BaseModel):
    x_min: float | None = Field(
        None, description="Grid X minimum (auto from data if None)"
    )
    x_max: float | None = Field(None, description="Grid X maximum")
    y_min: float | None = Field(None, description="Grid Y minimum")
    y_max: float | None = Field(None, description="Grid Y maximum")
    resolution: float = Field(1.0, gt=0, description="Grid cell size")
    nx: int | None = Field(None, description="Number of X cells (overrides resolution)")
    ny: int | None = Field(None, description="Number of Y cells")

    buffer: float = Field(
        0.0, ge=0, description="Buffer around data extent (in data units)"
    )


class KrigingConfig(BaseModel):
    method: Literal["ordinary", "simple", "universal", "indicator", "cokriging"] = (
        Field("ordinary", description="Kriging method")
    )

    # Universal kriging specific
    drift_terms: Literal["linear", "quadratic"] = Field(
        "linear", description="Drift terms for universal kriging"
    )

    # Simple kriging specific
    mean: float | None = Field(
        None, description="Known mean for simple kriging (auto if None)"
    )

    # Indicator kriging specific
    thresholds: list[float] | None = Field(
        None, description="Thresholds for indicator kriging"
    )

    neighborhood: NeighborhoodConfig = Field(
        default_factory=NeighborhoodConfig, description="Neighborhood search"
    )
...
class ValidationConfig(BaseModel):
    ...
    cv_method: Literal["loo", "kfold", "spatial"] = Field(
        "loo", description="Cross-validation method"
    )
    n_folds: int = Field(5, ge=2, description="Number of folds for k-fold CV")

    metrics: list[Literal["rmse", "mae", "mse", "r2", "bias", "mape"]] = Field(
        ["rmse", "mae", "r2"], description="Metrics to compute"
    )
...
class PlotConfig(BaseModel):
    ...
    figsize: tuple | None = Field(None, description="Figure size (width, height)")
    colormap: str = Field("viridis", description="Colormap name")
    title: str | None = Field(None, description="Custom plot title")


class VisualizationConfig(BaseModel):
    style: Literal["minimalist", "default", "seaborn"] = Field(
        "minimalist", description="Plot style"
    )
    ...
    plots: list[
        ...
        ["variogram", "kriging_map", "cross_validation"], description="Plots to create"
    )

    # Global plot settings
    dpi: int = Field(300, ge=72, description="Default DPI for all plots")
    colormap: str = Field("viridis", description="Default colormap")
    save_format: list[Literal["png", "pdf", "svg", "jpg"]] = Field(
        ["png"], description="Output formats"
    )

    # Per-plot configurations
    variogram: PlotConfig = Field(
        default_factory=PlotConfig, description="Variogram plot config"
    )
    kriging_map: PlotConfig = Field(
        default_factory=PlotConfig, description="Kriging map config"
    )
    variance_map: PlotConfig = Field(
        default_factory=PlotConfig, description="Variance map config"
    )
    cross_validation: PlotConfig = Field(
        default_factory=PlotConfig, description="CV plot config"
    )
...
class AnalysisConfig(BaseModel):
    ...
    preprocessing: PreprocessingConfig = Field(
        default_factory=PreprocessingConfig, description="Preprocessing"
    )
    variogram: VariogramConfig = Field(
        default_factory=VariogramConfig, description="Variogram modeling"
    )
    kriging: KrigingConfig = Field(
        default_factory=KrigingConfig, description="Kriging configuration"
    )
    validation: ValidationConfig = Field(
        default_factory=ValidationConfig, description="Validation"
    )
    visualization: VisualizationConfig = Field(
        default_factory=VisualizationConfig, description="Visualization"
    )
    output: OutputConfig = Field(
        default_factory=OutputConfig, description="Output configuration"
    )
    constants: Optional["ConstantsConfig"] = Field(
        default=None, description="Constants overrides"
    )
    ...
    def validate_config(self):
        ...
        if self.kriging.method == "cokriging" and self.data.z_secondary is None:
            raise ValueError("z_secondary must be provided for cokriging")

        # Check indicator kriging requirements
        if self.kriging.method == "indicator" and self.kriging.thresholds is None:
            raise ValueError("thresholds must be provided for indicator kriging")
