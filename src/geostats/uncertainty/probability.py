...
def probability_map(
    ...
    for i in range(n_realizations):
        z_sim = sgs.simulate(x_pred, y_pred, n_realizations=1, seed=i)
        z_sim = z_sim[0]  # Get first realization
        exceedance_count += comparison_func(z_sim, threshold).astype(float)
