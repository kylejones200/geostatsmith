...
- epsilon(s): spatial residuals modeled by kriging

Prediction:
 Z*(s0) = m*(s0, X0) + epsilon*(s0)

where:
- m*(s0, X0): ML prediction at s0
- epsilon*(s0): kriged residual at s0
...
class RegressionKriging(BaseKriging):
    ...
    def fit(
        ...
        logger.info(
            ...
            f"nugget={params.get('nugget', 0):.4f}, sill={params.get('sill', 0):.4f}, "
            f"range={params.get('range', 0):.2f}"
        )
