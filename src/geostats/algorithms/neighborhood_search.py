...
class NeighborhoodConfig:
    """Configuration for neighborhood search"""
...
class NeighborhoodSearch:
    ...
    def find_neighbors(
        ...
        if self.config.search_radius is not None:
            ...
            # Use entire dataset (or large radius)
            radius = np.inf
        
        # Query KD-tree
        if np.isfinite(radius):
            indices = self.kdtree.query_ball_point([x0, y0], radius)
            indices = np.array(indices, dtype=np.int64)
            if len(indices) == 0:
                return np.array([], dtype=np.int64), np.array([], dtype=np.float64)
            distances = np.linalg.norm(self.points[indices] - [x0, y0], axis=1)
        else:
            # Query for max_neighbors closest points
        ...
        if self.config.search_ellipse is not None:
            mask = self._in_search_ellipse(x0, y0, indices)
            indices = indices[mask]
            distances = distances[mask]
        
        # Apply octant or quadrant search
        if self.config.use_octants:
            indices, distances = self._octant_search(x0, y0, indices, distances)
        elif self.config.use_quadrants:
            indices, distances = self._quadrant_search(x0, y0, indices, distances)
        ...
        if len(indices) < self.config.min_neighbors:
            # If not enough, try to find more (expand search)
            if self.config.search_radius is not None:
                # Find nearest min_neighbors regardless of radius
    ...
    def _octant_search(
        ...
        for octant in range(8):
            octant_mask = (octants == octant)
            octant_idx = indices[octant_mask]
            octant_dist = distances[octant_mask]
            
            if len(octant_idx) > 0:
                # Sort by distance and take closest max_per_octant
    ...
    def _quadrant_search(
        ...
        for quadrant in range(4):
            quad_mask = (quadrants == quadrant)
            quad_idx = indices[quad_mask]
            quad_dist = distances[quad_mask]
            
            if len(quad_idx) > 0:
                sort_idx = np.argsort(quad_dist)
