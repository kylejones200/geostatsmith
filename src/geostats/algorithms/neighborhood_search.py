 ...
 """
    ...
    def find_neighbors(
        ...
        if np.isfinite(radius):
            indices, distances = self.kdtree.query_radius(
                [[x0, y0]], r=radius, return_distance=True, sort_results=True
            )
            indices = np.array(indices[0], dtype=np.int64)
            if len(indices) == 0:
                distances = np.array([], dtype=np.float64)
            else:
                distances = np.array(distances[0], dtype=np.float64)
        else:
