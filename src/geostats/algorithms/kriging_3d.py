...
class SimpleKriging3D(BaseKriging):
    ...
    def _build_kriging_matrix(self):
        """Build the 3D simple kriging covariance matrix (vectorized)"""
        n = len(self.x)
    ...
    def predict(
        ...
        for i in range(n_pred):
            ...
            # Solve for weights
            try:
                lambdas = solve_kriging_system(self.kriging_matrix, c0)
            ...
            if return_variance:
                variances[i] = sill - np.dot(lambdas, c0)
...
class OrdinaryKriging3D(BaseKriging):
    ...
    def _build_kriging_matrix(self):
        """Build 3D ordinary kriging matrix with unbiasedness constraint (vectorized)"""
        n = len(self.x)
    ...
    def predict(
        ...
        # Still need loop over prediction points for solving (inherent to kriging)
        for i in range(n_pred):
            # Build RHS vector (vectorized variogram lookup)
    ...
    def cross_validate(self) -> Tuple[npt.NDArray[np.float64], Dict[str, float]]:
        """Leave-one-out cross-validation for 3D kriging"""
        n = len(self.x)
