...
where fⱼ are basis functions (e.g., polynomials).
...
class UniversalKriging(BaseKriging):
    ...
    def _build_drift_matrix(
        ...
        if self.drift_terms == "linear":
            ...
            raise ValueError(f"Unknown drift terms: {self.drift_terms}")
        
        return F
    
    def _build_kriging_matrix(self) -> None:
        """Build the augmented kriging matrix with drift constraints"""
