...
class BlockKriging(BaseKriging):
 ...
 self._precompute_block_variance()
 ...
 raise KrigingError("Variogram model required for block kriging")
 ...
 # Build RHS with point-to-block variograms (still needs loop for each pred point)
 ...
 # Solve
 try:
 weights = solve_kriging_system(K, rhs)
 ...
 # σ²(V) = -γ(V,V) - ΣΣ λi λj γ(ui-uj) + 2Σ λi γ(ui,V)
 if return_variance:
 variances[i] = np.dot(weights, rhs) - self.gamma_VV
...
class SupportCorrection:
 ...
 variogram_point: Callable[[float], float],
 ...
 """Block variogram at separation distance h"""
 h = np.asarray(h)
 ...
 variogram: Callable[[float], float],
