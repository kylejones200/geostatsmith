...
def validate_coordinates(
    ...
    if z is not None:
        ...
        if x.shape != z.shape:
            raise ValidationError(
                f"X and Z coordinates must have same shape, got {x.shape} and {z.shape}"
            )
        return (x, y, z)
    ...
    *arrays : np.ndarray
    ...
    for i, shape in enumerate(shapes[1:], 1):
        if shape != first_shape:
            if names:
                msg = f"{names[0]} and {names[i]} must have same shape, got {first_shape} and {shape}"
            else:
                msg = f"Arrays must have same shape, got {first_shape} and {shape}"
