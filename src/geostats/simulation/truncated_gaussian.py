...
class TruncatedGaussianSimulation:
    ...
    def _setup_categories(self):
        """Setup category labels and proportions"""
        ...
        if self.config.proportions is not None:
            ...
            if not np.isclose(np.sum(self.config.proportions), 1.0):
                raise SimulationError("Proportions must sum to 1.0")
    ...
    def _calculate_thresholds(self):
        ...
        if self.config.thresholds is not None:
            self.thresholds = np.array(self.config.thresholds, dtype=np.float64)
    ...
    def simulate(
        ...
        if self.variogram_model is None:
            raise SimulationError("Variogram model required for simulation")
