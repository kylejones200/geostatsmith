...
class TruncatedGaussianSimulation:
 ...
 """
    ...
    def _calculate_thresholds(self):
        ...
        For k categories with proportions [p1, p2, ..., pk]:
        - Threshold t1 = Phi^-1(p1)
        - Threshold t2 = Phi^-1(p1 + p2)
        - ...
        - No threshold needed for last category (goes to +inf)
        ...
        if self.config.thresholds is not None:
            self.thresholds = np.array(self.config.thresholds)
    ...
    def simulate(
        ...
        if self.variogram_model is None:
            raise ValueError("variogram_model must be provided")
    ...
    def _apply_thresholds(
        ...
        categories : np.ndarray (int)
            Category labels
        """
        # Fully vectorized using np.digitize
