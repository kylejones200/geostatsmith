...
class PlurigaussianSimulation:
    ...
    def __init__(
        ...
        if len(self.x) != len(self.y) or len(self.x) != len(self.categories):
            raise ValueError("x, y, and categories must have the same length")
        ...
        if config.rule_function is None:
            raise ValueError("rule_function must be provided")
    ...
    def _setup_categories(self):
        ...
        if self.config.categories is not None:
            self.unique_categories = np.asarray(self.config.categories)
        else:
        ...
        for i in range(n_cond):
            ...
            for attempt in range(max_attempts):
                if self.config.random_seed is not None:
                    np.random.seed(self.config.random_seed + i * max_attempts + attempt)
                else:
                    pass
                ...
                if n_fields > 1:
                    test_cat = self.rule_function(*y_samples)
                else:
                    test_cat = self.rule_function(y_samples[0])

                if test_cat == cat:
                    for j in range(n_fields):
                        self.gaussian_fields_cond[j][i] = y_samples[j]
                    found = True
                    break
    ...
    def simulate(
        ...
        for r in range(self.config.n_realizations):
            ...
            if len(gaussian_realizations) > 1:
                categorical_realization = self.rule_function(*gaussian_realizations)
            else:
                categorical_realization = self.rule_function(gaussian_realizations[0])
        ...
        if len(original_shape) > 1:
            realizations_categorical = realizations_categorical.reshape(
...
def create_rectangular_rule(
    ...
    Partitions the (Y1, Y2) plane into rectangular regions.
    ...
    thresholds_y1 : list of float
        Thresholds for Y1 axis
    thresholds_y2 : list of float
        Thresholds for Y2 axis
    layout : str
    ...
    if layout == "grid":
        ...
        def grid_rule(y1, y2):
            ...
            categories = list(range(1, n_bins_y2 * (len(thresholds_y1) + 1) + 1))

            for i in range(n):
                cat_idx = bins_y1[i] * n_bins_y2 + bins_y2[i]
                if cat_idx < len(categories):
                    result[i] = categories[cat_idx]
                else:
                    result[i] = categories[-1]
