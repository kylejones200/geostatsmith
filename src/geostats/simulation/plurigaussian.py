...
class PlurigaussianConfig:
    """Configuration for Plurigaussian Simulation"""
...
class PlurigaussianSimulation:
    ...
    def __init__(
        ...
        if len(self.x) != len(self.y) or len(self.x) != len(self.categories):
            raise SimulationError("x, y, and categories must have same length")
        ...
        if config.rule_function is None:
            raise SimulationError("rule_function must be provided")
    ...
    def _setup_categories(self):
        ...
        if self.config.categories is not None:
            self.unique_categories = np.array(self.config.categories, dtype=np.int32)
        else:
        ...
        for i in range(n_cond):
            ...
            for attempt in range(max_attempts):
                ...
                if self.config.random_seed is not None:
                    np.random.seed(self.config.random_seed + i * n_fields + attempt)
    ...
    def simulate(
        ...
        for r in range(self.config.n_realizations):
            # Generate independent Gaussian realizations for each field
            ...
            if (r + 1) % 10 == 0:
                logger.debug(f"Completed {r + 1}/{self.config.n_realizations} realizations")
    ...
    def get_proportions_summary(
        ...
        for cat in self.unique_categories:
            # Calculate proportions for each realization
