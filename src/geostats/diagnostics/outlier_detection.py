    ...
    if method == "iqr":
        ...
        for i, (x_val, y_val, z_val) in enumerate(zip(x, y, z)):
            ...
            distances, indices = tree.query(
                [x_val, y_val], k=SPATIAL_QUERY_K_NEIGHBORS
            )  # Includes self
...
def robust_validation(
    ...
    for i in range(n):
        ...
        krig = OrdinaryKriging(
            x[train_idx], y[train_idx], z[train_idx], variogram_model
        )
    ...
    if len(outlier_idx) > 0:
        ...
        for i in range(n_clean):
            train_idx = np.setdiff1d(np.arange(n_clean), [i])
            krig = OrdinaryKriging(
                x_clean[train_idx],
                y_clean[train_idx],
                z_clean[train_idx],
            ...
            pred, _ = krig.predict(
                np.array([x_clean[i]]), np.array([y_clean[i]]), return_variance=True
            )
    ...
    return {
        ...
        "improvement": float((rmse_all - rmse_clean) / rmse_all * 100)
        if rmse_all > 0
        else 0.0,
    }
