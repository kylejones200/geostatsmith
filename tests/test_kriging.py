...
def test_block_kriging():
 ...
 x_flat = X.flatten()
 y_flat = Y.flatten()
 z_flat = np.sin(x_flat / 20) + np.cos(y_flat / 20) + np.random.randn(len(x_flat)) * 0.1
 ...
 vario_model = variogram.fit_model('exponential', lags, gamma)
 ok = kriging.OrdinaryKriging(x_sample, y_sample, z_sample, variogram_model=vario_model)
