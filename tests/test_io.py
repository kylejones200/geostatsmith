...
class TestCSVIO:
    ...
    def test_read_csv_spatial_custom_columns(self):
        ...
        x_read, y_read, z_read, _ = read_csv_spatial(
            str(csv_file), x_col="lon", y_col="lat", z_col="value"
        )
    ...
    def test_read_csv_spatial_with_additional_cols(self):
        ...
        df = pd.DataFrame(
            {"x": self.x, "y": self.y, "z": self.z, "covariate": extra_data}
        )
        csv_file = self.temp_dir / "test.csv"
        df.to_csv(csv_file, index=False)

        x_read, y_read, z_read, extra = read_csv_spatial(
            str(csv_file), additional_cols=["covariate"]
        )
    ...
    def test_write_csv_spatial_basic(self):
        ...
        write_csv_spatial(str(csv_file), self.x, self.y, self.z)
    ...
    def test_write_csv_spatial_with_extra(self):
        ...
        write_csv_spatial(str(csv_file), self.x, self.y, self.z, extra=extra)
...
class TestRasterIO:
    ...
    def test_write_read_geotiff(self):
        ...
        # Write
        write_geotiff(
            str(tif_file), self.x_grid, self.y_grid, self.z_grid, crs="EPSG:4326"
        )
    ...
    def test_write_geotiff_handles_nan(self):
        ...
        write_geotiff(
            str(tif_file), self.x_grid, self.y_grid, z_with_nan, nodata=-9999.0
        )
...
class TestNetCDFIO:
    ...
    def test_write_read_netcdf(self):
        ...
        x_read, y_read, z_read, meta = read_netcdf(
            str(nc_file), z_var="temperature", x_var="longitude", y_var="latitude"
        )
