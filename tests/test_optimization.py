...
class TestSamplingDesign:
    ...
    def setup_method(self):
        ...
        self.z_existing = 50 + 0.3 * self.x_existing + np.random.normal(0, 3, self.n_existing)
    ...
    def test_optimal_sampling_design_variance_reduction(self):
        ...
        x_new, y_new = optimal_sampling_design(
            self.x_existing, self.y_existing, self.z_existing,
            n_new_samples=5,
...
class TestCostBenefit:
    ...
    def setup_method(self):
        ...
        self.z_initial = 50 + 0.3 * self.x_initial + np.random.normal(0, 3, self.n_initial)
    ...
    def test_sample_size_calculator_basic(self):
        ...
        results = sample_size_calculator(
            self.x_initial, self.y_initial, self.z_initial,
            variogram_model=self.model,
            target_rmse=2.0,
            max_samples=100,
    ...
    def test_cost_benefit_analysis_different_costs(self):
        ...
        assert results1['optimal_samples'] != results2['optimal_samples'] or \
               results1['net_benefit'] != results2['net_benefit']
    ...
    def test_cost_benefit_analysis_net_benefit_properties(self):
        ...
        optimal_idx = np.argmin(np.abs(results['sample_sizes'] - results['optimal_samples']))
        assert net_benefits[optimal_idx] >= np.max(net_benefits) - 0.1  # Allow small tolerance
