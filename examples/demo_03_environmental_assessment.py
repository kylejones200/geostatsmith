...
print("="*80)
print("🛡️  ENVIRONMENTAL GUARDIAN - CONTAMINATION RISK ASSESSMENT")
print("="*80)
...
def load_environmental_data(agdb_path, elements=['As', 'Pb', 'Hg']):
    ...
    print(f"\n🌍 Loading environmental data ({', '.join(elements)})...")
    ...
    for element in elements:
        ...
        print(f"  {element}: {len(merged):,} samples")
...
def analyze_thresholds(data_dict):
    ...
    print("\n⚖️  Regulatory Threshold Analysis...")
    ...
    for idx, element in enumerate(['As', 'Pb', 'Hg']):
        ...
        print(f"\n  {element}:")
        print(f"    Mean: {values.mean():.2f} {thresh['units']}")
        print(f"    Median: {np.median(values):.2f} {thresh['units']}")
        print(f"    Max: {values.max():.2f} {thresh['units']}")
        print(f"    > Background: {pct_exceed:.1f}%")
    
    plt.tight_layout()
    plt.savefig('alaska_threshold_analysis.png', dpi=150)
    print("\n  📊 Saved: alaska_threshold_analysis.png")
...
def create_exceedance_maps(data_dict, thresholds):
    ...
    for idx, element in enumerate(['As', 'Pb', 'Hg']):
        ...
        print(f"\n  {element}: P(>{threshold} {thresholds[element]['units']})...")
        ...
        print(f"    High risk (>90%): {high_risk:.1f}% of area")
        print(f"    Medium risk (50-90%): {medium_risk:.1f}% of area")
    
    plt.tight_layout()
    plt.savefig('alaska_exceedance_probability.png', dpi=150)
    print("\n  📊 Saved: alaska_exceedance_probability.png")
...
def multi_threshold_risk_assessment(data_dict, element='As'):
    ...
    print(f"\n🚨 Multi-Threshold Risk Assessment ({element})...")
    
    if element not in data_dict:
        print(f"  {element} data not available")
        return
    ...
    print(f"\n  📊 Saved: alaska_{element.lower()}_risk_classification.png")
...
def identify_hotspots(data_dict):
    ...
    print("\n🔥 Contamination Hotspot Identification...")
    ...
    for idx, element in enumerate(['As', 'Pb', 'Hg']):
        ...
        print(f"\n  {element} Hotspots:")
        print(f"    Threshold: >{threshold:.2f} ppm")
        print(f"    Locations: {n_hotspots}")
    ...
    print("\n  📊 Saved: alaska_contamination_hotspots.png")
...
def generate_environmental_report(data_dict, thresholds, hotspots):
    ...
    if 'As' not in data_dict:
        print("  Arsenic data not available for report")
        return
    ...
    print("  ✅ Report generated: alaska_environmental_report.html")
    print("  📧 Client-ready professional report!")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("\n" + "="*80)
        print("🎉 COMPLETE! Generated 4 Environmental Analysis Outputs:")
        print("="*80)
        print("  1. alaska_threshold_analysis.png - Regulatory comparison")
        print("  2. alaska_exceedance_probability.png - Probability maps (As, Pb, Hg)")
        print("  3. alaska_as_risk_classification.png - Multi-threshold risk")
        print("  4. alaska_contamination_hotspots.png - Priority locations")
        print("  5. alaska_environmental_report.html - Professional report")
        print("\n✨ This demo showcased:")
        print("  ✓ Environmental threshold analysis (EPA standards)")
        print("  ✓ Probability of exceedance mapping")
        print("  ✓ Multi-threshold risk classification")
        print("  ✓ Contamination hotspot identification")
        print("  ✓ Professional HTML reporting")
        print("\n🛡️  Environmental Geochemistry = Informed Decision Making!")
