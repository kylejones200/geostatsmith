...
def main():
    ...
    spatial_correlation = np.random.multivariate_normal(
        np.zeros(n_samples),
        np.exp(-np.sqrt((x[:, None] - x[None, :])**2 + (y[:, None] - y[None, :])**2) / 20)
    )
    z = np.exp(2 + 0.5 * spatial_trend + 0.3 * spatial_correlation + np.random.normal(0, 0.2, n_samples))
    ...
    axes[0, 1].scatter(x, y, c='white', s=20, alpha=0.6, edgecolors='black', linewidth=0.5)
    ...
    axes[1, 0].scatter(x, y, c='white', s=20, alpha=0.6, edgecolors='black', linewidth=0.5)
