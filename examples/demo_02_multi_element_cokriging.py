...
print("="*80)
print("⚗️  MULTI-ELEMENT ARSENAL - PORPHYRY DEPOSIT ANALYSIS")
print("="*80)
...
def load_multi_element_data(agdb_path, region='Pebble'):
    ...
    """
    print("\n📊 Loading multi-element data (Cu-Mo-Au)...")
    ...
    if region and 'QUAD' in merged.columns:
        ...
        if len(region_data) > 50:
            merged = region_data
            print(f"  Focused on {region} region")
    
    print(f"  Samples with Cu+Mo+Au: {len(merged):,}")
...
def analyze_element_correlations(data):
    ...
    print(f"\n  Element Correlations (log-transformed):")
    print(f"    Cu-Mo: r={corr_cu_mo:.3f} (p={p_cu_mo:.4f})")
    print(f"    Cu-Au: r={corr_cu_au:.3f} (p={p_cu_au:.4f})")
    print(f"    Mo-Au: r={corr_mo_au:.3f} (p={p_mo_au:.4f})")
    
    if corr_cu_mo > 0.5:
        print(f"  ✨ STRONG Cu-Mo correlation - classic porphyry signature!")
    ...
    print("\n  📊 Saved: alaska_element_correlations.png")
...
def detect_geochemical_anomalies(data, elements_dict):
    ...
    for element in ['Cu', 'Mo', 'Au']:
        ...
        print(f"\n  {element} Anomalies:")
        print(f"    Detected: {n_anomalies} ({n_anomalies/len(values)*100:.1f}%)")
        print(f"    Threshold: >{outlier_results['threshold']:.2f} ppm")
        print(f"    Max value: {values.max():.2f} ppm")
    ...
    print(f"\n  🎯 MULTI-ELEMENT ANOMALIES (Cu+Mo+Au):")
    print(f"    Samples: {n_combined} ({n_combined/len(combined_anomaly)*100:.2f}%)")
    print(f"    ⭐ These are high-priority exploration targets!")
    ...
    print("\n  📊 Saved: alaska_anomaly_detection.png")
...
def compare_kriging_vs_cokriging(data, elements_dict):
    ...
    print("\n  2️⃣  Cokriging (Cu using Mo as secondary variable)...")
    ...
    print(f"\n  📉 Average variance reduction: {variance_reduction:.1f}%")
    print(f"  ✨ Cokriging provides {variance_reduction:.0f}% more certain predictions!")
    ...
    print("\n  📊 Saved: alaska_cokriging_comparison.png")
...
def calculate_porphyry_index(data, elements_dict):
    ...
    print(f"  Index range: {porphyry_index.min():.3f} to {porphyry_index.max():.3f}")
    ...
    print(f"\n  🎯 High-potential zones: {high_potential.sum() / high_potential.size * 100:.1f}% of area")
    print(f"  ⭐ Top 10% porphyry fertility")
    ...
    print("\n  📊 Saved: alaska_porphyry_index.png")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("\n" + "="*80)
        print("🎉 COMPLETE! Generated 4 Advanced Analysis Figures:")
        print("="*80)
        print("  1. alaska_element_correlations.png - Cu-Mo-Au associations")
        print("  2. alaska_anomaly_detection.png - Multi-element anomaly maps")
        print("  3. alaska_cokriging_comparison.png - OK vs Cokriging")
        print("  4. alaska_porphyry_index.png - Integrated fertility index")
        print("\n✨ This demo showcased:")
        print("  ✓ Multi-element geochemistry")
        print("  ✓ Element correlation analysis")
        print("  ✓ Anomaly detection (3 methods)")
        print("  ✓ Cokriging for improved predictions")
        print(f"  ✓ {var_reduction:.0f}% variance reduction with cokriging!")
        print("  ✓ Porphyry fertility indexing")
        print("\n⚗️  Multi-Element Analysis = Powerful Exploration Tool!")
