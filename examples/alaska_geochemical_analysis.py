...
Database (AGDB4), a comprehensive database of geochemical analyses from Alaska.
...
def load_agdb4_data(agdb_path, element='Au', sample_type='stream sediment'):
    ...
    print(f"Loading AGDB4 data for {element}...")
    ...
    print(f"  Loaded {len(geol_data):,} samples from Geol_DeDuped")
    ...
    print(f"  Loaded {len(chem_data):,} analyses from {chem_filename}")
    ...
    if 'VALUE' in merged.columns:
        ...
        print(f"  Warning: Could not find value column for {element}")
        value_col = None
    ...
    print(f"  After filtering: {len(merged):,} samples with valid {element} data")
...
def gold_exploration_analysis(agdb_path, region_name='Iliamna'):
    ...
    """
    print("\n" + "="*70)
    print("GOLD EXPLORATION ANALYSIS")
    print("="*70 + "\n")
    ...
    if region_name:
        ...
        if len(indices) > 0:
            ...
            print(f"Focused on {region_name} region: {len(indices)} samples")
    ...
    print(f"  Range: {model['range']:.2f} degrees")
    ...
    print(f"\nExploration Targets:")
    print(f"  High potential area: {high_potential.sum() / high_potential.size * 100:.1f}% of region")
    print(f"  Max predicted Au: {au_pred.max():.3f} ppm")
...
def multi_element_analysis(agdb_path):
    ...
    """
    print("\n" + "="*70)
    print("MULTI-ELEMENT COKRIGING ANALYSIS (Cu-Au-Mo)")
    print("="*70 + "\n")
    ...
    print(f"Common Cu-Mo samples: {len(common)}")
    
    if len(common) < 50:
        print("Not enough common samples for cokriging demo")
        return None
    ...
    print(f"  Cu-Mo correlation: {correlation:.3f}")
    
    if correlation > 0.3:
        print("  Strong correlation detected - good for cokriging!")
    ...
    print(f"  Cu variogram range: {model_cu['range']:.2f}°")
    print(f"  Mo variogram range: {model_mo['range']:.2f}°")
...
def environmental_assessment(agdb_path, element='As', threshold=20):
    ...
    """
    print("\n" + "="*70)
    print(f"{element.upper()} ENVIRONMENTAL ASSESSMENT")
    print("="*70 + "\n")
    ...
    print(f"\n{element} Statistics:")
    print(f"  Mean: {values.mean():.2f} ppm")
    print(f"  Median: {np.median(values):.2f} ppm")
    print(f"  Max: {values.max():.2f} ppm")
    print(f"  >{threshold} ppm: {(values > threshold).sum()} samples " + 
          f"({(values > threshold).sum()/len(values)*100:.1f}%)")
    ...
    print(f"  Low risk (<10%): {(risk==0).sum()/risk.size*100:.1f}%")
    print(f"  Medium risk (10-50%): {(risk==1).sum()/risk.size*100:.1f}%")
    print(f"  High risk (>50%): {(risk==2).sum()/risk.size*100:.1f}%")
...
if __name__ == '__main__':
    ...
    if not AGDB_PATH.exists():
        print(f"Error: AGDB4 path not found: {AGDB_PATH}")
        print("Please download AGDB4 and update the path in this script.")
        exit(1)
    
    print("="*70)
    print("ALASKA GEOCHEMICAL DATABASE (AGDB4) ANALYSIS")
    print("Using GeoStats Library")
    print("="*70)
    ...
    print("  ✓ Loading large-scale geochemical databases")
    print("  ✓ Mineral exploration workflows (Au)")
    print("  ✓ Multi-element cokriging (Cu-Mo)")
    print("  ✓ Environmental risk assessment (As)")
    print("  ✓ Probability mapping and uncertainty quantification")
