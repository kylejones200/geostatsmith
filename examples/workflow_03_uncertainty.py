...
def example_1_bootstrap_confidence():
    ...
    print("Computing bootstrap uncertainty (100 iterations)...")
    ...
    # Confidence bands
    ax.fill_between(x_pred, results['lower_bound'], results['upper_bound'],
                     alpha=0.3, color='blue', label='95% CI')
    ax.fill_between(x_pred, results['percentile_2.5'], results['percentile_97.5'],
                     alpha=0.2, color='lightblue', label='95% Percentile')
    
    # Original samples (only those near the transect)
    mask = np.abs(y - 50) < 10
    ax.scatter(x[mask], z[mask], c='red', s=100, marker='o', 
               label='Samples', edgecolor='k', zorder=5)
    ...
    print("✓ Saved example_workflow_03_bootstrap.png")
    plt.close()
...
def example_2_probability_map():
    ...
    print("Creating contamination scenario...")
    np.random.seed(42)
    n = 50
    x = np.random.uniform(0, 100, n)
    y = np.random.uniform(0, 100, n)
    ...
    print("Computing probability map (50 realizations)...")
    ...
    print(f"✓ Probability map complete")
    print(f"  Max probability: {prob.max():.2%}")
    print(f"  Area with P>50%: {(prob > 0.5).sum() / len(prob):.1%}")
    ...
    ax1.contour(x_grid, y_grid, z_pred_grid, levels=[12], colors='black', 
    ...
    # Probability map
    im2 = ax2.contourf(x_grid, y_grid, prob_grid, levels=np.linspace(0, 1, 11), cmap='RdYlGn_r')
    ax2.contour(x_grid, y_grid, prob_grid, levels=[0.5], colors='black', 
    ...
    print("✓ Saved example_workflow_03_probability.png")
    plt.close()
...
def example_3_risk_assessment():
    ...
    print("  Threshold (regulatory limit): 12")
    print("  Cost of false positive (unnecessary remediation): $10,000")
    print("  Cost of false negative (health risk): $100,000")
    ...
    print(f"  Recommend remediation: {n_remediate} cells ({n_remediate/len(x_pred)*100:.1f}%)")
    print(f"  Recommend no action: {n_no_action} cells ({n_no_action/len(x_pred)*100:.1f}%)")
    ...
    im1 = axes[0].contourf(x_grid, y_grid, prob_grid, levels=np.linspace(0, 1, 11), cmap='RdYlGn_r')
    ...
    decision_numeric = (results['optimal_decision'] == 'positive').astype(int).reshape((ny, nx))
    im3 = axes[2].contourf(x_grid, y_grid, decision_numeric, levels=[0, 0.5, 1], 
                            colors=['lightgreen', 'salmon'], alpha=0.7)
    ...
    print("✓ Saved example_workflow_03_risk.png")
    plt.close()
...
def main():
    """Run all examples."""
    print("\n" + "="*70)
    print("GEOSTATS UNCERTAINTY QUANTIFICATION EXAMPLES")
    print("="*70)
    ...
    print("="*70)
    print("\nFiles created:")
    print("  - example_workflow_03_bootstrap.png")
    print("  - example_workflow_03_probability.png")
    print("  - example_workflow_03_risk.png")
    print("\n")
