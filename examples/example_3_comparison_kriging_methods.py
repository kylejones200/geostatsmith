...
print("Generating data with spatial trend...")
np.random.seed(123)
n = 100
...
print(f"Generated {n} points with trend")
...
uk = kriging.UniversalKriging(x, y, z, variogram_model=vario_model, drift_terms='linear')
...
print("-" * 50)
print(f"{'Simple Kriging':<20} {metrics_sk['rmse']:<10.4f} {metrics_sk['mae']:<10.4f} {metrics_sk['r2']:<10.4f}")
print(f"{'Ordinary Kriging':<20} {metrics_ok['rmse']:<10.4f} {metrics_ok['mae']:<10.4f} {metrics_ok['r2']:<10.4f}")
print(f"{'Universal Kriging':<20} {metrics_uk['rmse']:<10.4f} {metrics_uk['mae']:<10.4f} {metrics_uk['r2']:<10.4f}")
...
for idx, (method_name, z_pred, var_pred, metrics) in enumerate(methods, 1):
    ...
    ax1.scatter(x, y, c=z, s=30, cmap='viridis', edgecolors='white', linewidth=0.5, zorder=5)
...
print("Saved plot to: example_3_comparison.png")
plt.show()
