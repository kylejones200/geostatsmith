...
try:
    from geostats.algorithms.performance import (
        ChunkedKriging,
        CachedKriging,
        approximate_kriging,
    )
...
def example_1_parallel_kriging():
    ...
    logger.info(f" Speedup: {time_seq/time_par:.1f}x")
...
def example_3_caching():
    ...
    logger.info(f" Speedup: {time1/time2:.0f}x (instant!)")
...
def example_4_approximate_methods():
    ...
    logger.info(f" Time: {time_exact:.2f}s")
    ...
    logger.info(f" Time: {time_approx:.2f}s")
    logger.info(f" Speedup: {time_exact/time_approx:.1f}x")
...
def main():
    ...
    logger.info(" • Caching: Instant repeated predictions")
    logger.info(" • Approximate methods: 10-100x speedup with <5% error")
