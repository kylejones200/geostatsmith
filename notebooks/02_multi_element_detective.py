...
print("✅ Imports successful - Ready to hunt for porphyries!")
...
def load_porphyry_data():
    ...
    print(f"   Iliamna samples: {len(iliamna):,}")
...
print(f"\n📊 Element Statistics:")
print(f"\nCopper:")
print(f"   Mean: {cu.mean():.1f} ppm")
print(f"   Median: {np.median(cu):.1f} ppm")
print(f"   Max: {cu.max():.1f} ppm")
print(f"   >200 ppm: {(cu > 200).sum()} samples")

print(f"\nGold:")
print(f"   Mean: {au.mean():.3f} ppm")
print(f"   Median: {np.median(au):.3f} ppm")
print(f"   Max: {au.max():.3f} ppm")
print(f"   >0.2 ppm: {(au > 0.2).sum()} samples")
...
print(f"🔬 Element Correlations:")
print(f"   Cu-Au (raw): r = {corr_cu_au:.3f}")
print(f"   Cu-Au (log): r = {corr_log:.3f}")
print(f"\n💡 Interpretation:")
if corr_log > 0.5:
    print(f"   ✅ STRONG positive correlation!")
    print(f"   ✅ Characteristic of porphyry Cu-Au deposits")
    print(f"   ✅ Elements formed together from same hydrothermal system")
else:
    print(f"   ⚠️  Weak correlation - mixed sources?")
...
print("🗺️ Observation: High Cu and Au co-occur in central area (known Pebble deposit!)")
...
# %%
print("🔮 Performing ordinary kriging for Cu and Au separately...")
...
print("✅ Univariate kriging complete!")
...
# %%
print("⚡ Performing Cokriging (using Cu to improve Au predictions)...")
...
print("   Calculating cross-variogram...")
...
print(f"\nCopper:")
print(f"   IQR method: {cu_anom_iqr.sum()} anomalies ({cu_anom_iqr.sum()/len(cu)*100:.1f}%)")
print(f"   Top 5%: {cu_anom_p95.sum()} samples")
print(f"\nGold:")
print(f"   IQR method: {au_anom_iqr.sum()} anomalies ({au_anom_iqr.sum()/len(au)*100:.1f}%)")
print(f"   Top 5%: {au_anom_p95.sum()} samples")
print(f"\n⭐ Multi-Element Anomalies:")
print(f"   Both Cu AND Au enriched: {multi_anom.sum()} samples")
print(f"   These are HIGH-PRIORITY targets!")
