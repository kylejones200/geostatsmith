...
# 1. Gold distribution patterns across Fairbanks
...
def load_fairbanks_gold():
 ...
 logger.info(f" Total samples: {len(geol):,}")
 ...
 logger.info(f" Fairbanks samples: {len(fairbanks):,}")
...
logger.info(f" >1 ppm: {(au > 1.0).sum()} samples (economic grade!)")
...
logger.info(" Observation: Samples cluster near known mining areas (preferential sampling)")
...
logger.info(" Calculating experimental variogram...")
...
logger.info(f"Variogram Model:")
logger.info(f" Type: {model['model']}")
logger.info(f" Range: {model['range']:.3f}° ({model['range']*111:.1f} km)")
logger.info(f" Sill: {model['sill']:.3f}")
logger.info(f" Nugget: {model['nugget']:.3f}")
...
# %%
logger.info(" Performing Lognormal Kriging...")
...
# Statistics
logger.info(f"Target Classification:")
logger.info(f" Low probability: {(risk_class==0).sum()/risk_class.size*100:.1f}% of area")
logger.info(f" Moderate: {(risk_class==1).sum()/risk_class.size*100:.1f}% of area")
logger.info(f" HIGH PRIORITY: {(risk_class==2).sum()/risk_class.size*100:.1f}% of area")
...
# %%
logger.info(" Finding optimal infill sample locations...")
...
logger.info(f" Identified {n_new} optimal sampling locations!")
...
logger.info(f" Random sampling: ~{n_new*3} samples needed for same information")
logger.info(f" Optimal design: {n_new} samples (strategic locations)")
logger.info(f" Savings: ~{n_new*2} samples = ${n_new*2*500:,} USD")
logger.info(f" (assuming $500 per sample for assay + collection)")
...
# %%
logger.info(" Loading pathfinder elements (As, Sb)...")
